...
function isCitizen() {
  return document.location.pathname.startsWith("/citizen") || window.__CURRENT_APP__ === "CITIZEN";
}

function isEmployee() {
  return document.location.pathname.startsWith("/employee") || window.__CURRENT_APP__ === "EMPLOYEE";
}

function loadUCMiniReceiptButton(UCreceiptURL) {
  var UCfooterElem = document.getElementsByClassName("apply-wizard-footer")[0];

  if (UCfooterElem != null) {
    var UCminiReceiptButton = document.createElement("button");
    UCminiReceiptButton.innerHTML = '<a id=UCprintMiniReceiptLink ><button tabindex="0" class="jss104 jss78 jss89 jss90 jss92 jss93" type="button" style="min-width: 200px; height: 48px; margin-right: 16px;"><span class="jss79"><span data-localization="UC_BUTTON_VIEW_RECEIPT" id="custom-containers-downloadReceiptButtonLabel">MINI RECEIPT</span></span><span class="jss107"></span></button></a>';
    UCminiReceiptButton = UCminiReceiptButton.firstChild;
    UCminiReceiptButton.style.marginRight = "10px";
    UCminiReceiptButton.style.marginLeft = "10px";
    UCfooterElem.insertBefore(UCminiReceiptButton, UCfooterElem.firstChild);
    var UCminiReceiptLink = document.getElementById("UCprintMiniReceiptLink");
    UCminiReceiptLink.href = encodeURI(UCreceiptURL);
    UCminiReceiptLink.style.color = "#ffffff";
  } else setTimeout(loadUCMiniReceiptButton, 500, UCreceiptURL);
}
function UCminiReceiptBuilder(h) {
  var NEXTLINE = "&&";
